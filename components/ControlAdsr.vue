...
svg.min-w-45.max-w-55.m-1.touch-none(
  ...
  g.numbers(text-anchor="middle" font-size="5" dominant-baseline="middle" fill="currentColor" :transform="`translate(0 ${height * .25})`" )
    text(:x="width / 8") {{ a.toFixed(2) }}
    text(:x="3 * width / 8") {{ d.toFixed(2) }}
    text(:x="5 * width / 8") {{ s.toFixed(2) }}
    text(:x="7 * width / 8") {{ r.toFixed(2) }}

  g.titles(text-anchor="middle" font-size="4" dominant-baseline="middle" fill="currentColor" :transform="`translate(0 ${height * .75})`" )
    text(:x="width / 8") ATT
